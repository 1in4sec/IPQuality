...
echo "bash <(curl -sL https://raw.githubusercontent.com/1in4sec/IPQuality/main/ref/upgrade_bash.sh)"
exit 0
fi
}
check_bash
...
"Parameters: bash <(curl -sL https://IP.Check.Place) [-4] [-6] [-f] [-h] [-i iface] [-j] [-l language] [-n] [-o outputpath] [-p] [-x proxy] [-y] [-E] [-M]"
"            -4                             Test IPv4"
"            -6                             Test IPv6"
"            -f                             Show full IP on reports"
"            -h                             Help information"
"            -i eth0                        Specify network interface"
"               ipaddress                   Specify outbound IP address"
"            -j                             JSON output"
"            -l en|jp|es|de|fr|ru|pt        Specify report language"
"            -n                             Skip OS and dependency checks"
"            -o /path/to/file.ansi          Output ANSI report to file"
"               /path/to/file.json          Output JSON result to file"
"               /path/to/file.anyother      Output plain text report to file"
"            -p                             Privacy mode - do not generate report link"
"            -x http://usr:pwd@proxyurl:p   Specify HTTP proxy"
"               https://usr:pwd@proxyurl:p  Specify HTTPS proxy"
"               socks5://usr:pwd@socks:p    Specify SOCKS5 proxy"
"            -y                             Install dependencies without interruption"
"            -E                             Use English output"
"            -M                             Run with interactive interface")
shelp=$(printf "%s\n" "${shelp_lines[@]}")
set_language(){
case "$YY" in
...
shead[git]="https://github.com/1in4sec/IPQuality"
...
rawgithub="https://github.com/1in4sec/IPQuality/raw/"
return 0
else
rawgithub="https://testingcf.jsdelivr.net/gh/1in4sec/IPQuality@"
return 1
fi
}
is_valid_ipv4(){
